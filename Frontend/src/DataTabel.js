...
const StoreReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
       ...
       // Filter out rows where transection_id is null
       const filteredRows = rowsWithUniqueIds.filter(row => row.transection_id != "");
  ...
  const columns = [
    ...
    
    {
        ...
        headerClassName: 'custom-header-class',
        renderCell: (params) => {
          const currencySymbol = params.row.payment_type.toLowerCase() === 'razor' || params.row.payment_type.toLowerCase() === 'razorpay'
